    ...
    for i in range(state.shape[0]):
        ...
        if Inv_store > demand_real[i]:
            ...
            for new_state0 in range(1,Inv_store+1):
                side_state = torch.clone(state[i])
                side_state[0] = (new_state0-1)/(env.InvMax+env.LT_s*env.max_order)
                side_states.append(side_state)
...
def test_env(all_test_rewards):
    ...
    print("Test: ",episode,episode_reward/test_num_frames,np.average(all_test_rewards[-400:-1]),episode_in_reward/test_num_frames)
